// source: https://www.npmjs.com/package/react-bounding-box
/* global Image */
...
class Boundingbox extends Component {
  ...
  renderBox(box, index) {
    ...
    let labelColor = 'rgba(255,0,0,1)'
    if (this.state.hoverIndex >= 0) {
      color = this.props.options.colors.unselected;
      labelColor = this.props.options.colors.unselected
    }
    ...
    if(box.label) { this.props.drawLabel(this.canvas, box, labelColor) };
  }
...
Boundingbox.defaultProps = {
  ...
  drawLabel(canvas, box, color) {
    ...
    ctx.font = '60px Arial';
    ctx.fillStyle = color;
    ctx.fillText(box.label, x, height-10);
  },
  options: {
    colors: {
